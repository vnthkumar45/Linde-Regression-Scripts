...
public class f8_Involuntary_Separation_LGLCO extends AgentGenericMethods2 {
	
	public static void NewHire_Rehire_Eform_Verification_and_Submission(String URL, String ScreenshotPath,
		...
		ReadExcel(EformPath);
		try {
			String Login_sheet_name = "DE_Involuntary_Separation";
			...
			for (int i = 1; i <= totalRows; i++) {// declaring while loop to iterate in sheet untill we have rows
				...
				String TypeofInvoluntary= GetdataFromSheet(Login_sheet_name, "Please select the type of involuntary separation request", i);
				...
				if (EformName.isEmpty()) {
					...
					TypeOfRequestfor = TypeofInvoluntary;
					//MyHR - Submit Second Employer Eform
					Class1_MyHR_genericMethods.Populate_fields_and_Submit_Eform(Scenario,"Involuntary_Separation", RequestRaisedFor, MethodName);
					...
					if (TypeofInvoluntary.equalsIgnoreCase("Termination by company")
							|| TypeofInvoluntary.equalsIgnoreCase("Termination Agreement")) {
						...
						if (ParentTicket_Created == true) {
							...
							ApprovalTicket_ID = Class2_Agent_genericMethods.VerifyParentTicket_AfterIntiation(TicketID,
									HasApproval, Number_Of_Approval, Country, TypeOfApproval, MethodName);
							System.out.println(ApprovalTicket_ID);
						}

						System.out.println("First Approval triggered flag =" + First_Approval_Triggered_flag);

						if ((First_Approval_Triggered_flag == true)
								&& ((Number_Of_Approval.equalsIgnoreCase("2 - Approval"))
										|| (Number_Of_Approval.equalsIgnoreCase("1 - Approval")))) {
							// Agent -Verify the Approval Ticket ID 1
							Class2_Agent_genericMethods.Verify_Agent_ApprovalTicket(URL, ApprovalTicket_ID, EformName,
									EmployeeEmailAddress, MethodName);
						}
						...
						if (is_Approver_Details_correct == true) {
							...
							// MyHR -Login as Approver in MyHR and Verify the Approval Ticket and then
							// approve or reject the first approval
							Approval_Action = Class1_MyHR_genericMethods.Approver_Action_on_ApprovalTicket(URL,
									ApproverEmailID, Password, ApprovalTicket_ID, Country, TypeOfApproval, MethodName);
						...
						if (Approval_Action.equalsIgnoreCase("Done")) {
							...
							String ApprovalTicket_ID2 = Class2_Agent_genericMethods.VerifyParentTicket_AfterApproval(
									TicketID, HasApproval, Number_Of_Approval, Country, TypeOfApproval, MethodName);
						...
						if (ParentTicket_Created == true) {
							...
							// Agent - Parent Ticket Verification and get Approval Ticket
							Class2_Agent_genericMethods.VerifyParentTicket_AfterIntiation(TicketID, HasApproval,
									Number_Of_Approval, Country, TypeOfApproval, MethodName);
