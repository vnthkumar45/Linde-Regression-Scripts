...
public class f6_Transfer_DifferentEntity_Eform extends AgentGenericMethods2 {
	
	public static void TransferDifferentEntity_Eform_Verification_and_Submission(String URL, String ScreenshotPath,
		...
		ReadExcel(EformPath);
		try {
			String Login_sheet_name = "DE_Transfer_Different Entity";
			...
			for (int i = 1; i <= totalRows; i++) {// declaring while loop to iterate in sheet untill we have rows
				...
				if (EformName.isEmpty()) {
					...
					Class1_MyHR_genericMethods.Populate_fields_and_Submit_Eform(Scenario,"Transfer_DifferentEntity", RequestRaisedFor, MethodName);
