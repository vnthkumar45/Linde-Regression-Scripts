...
public class Class2_Agent_genericMethods extends AgentGenericMethods2 {
	...
	// Utility method for bulk fetching ticket details
		private static Map<String, String> fetchTicketDetails() throws Exception {
		    Map<String, String> details = new HashMap<>();
		    details.put("Requestor", AgentGetText("//div[@data-test-id='ticket-system-field-requester-select']"));
		    details.put("Assignee", AgentGetText("//div[@data-test-id='assignee-field-autocomplete-trigger']"));
		    details.put("Form", AgentGetText("//div[@data-test-id='ticket-fields-ticket-forms']/div/div"));
		    details.put("Country", AgentGetText("//label[text()='Country*']//following-sibling::div//span/div"));
		    details.put("Priority", AgentGetText("//div[@data-test-id='ticket-fields-priority-select']"));
		    details.put("HRTicketType", AgentGetText("//label[text()='HR Ticket Type*']//following-sibling::div//span/div"));
		    details.put("OpsCat1", AgentGetText("//label[text()='Ops Cat 1 (New)']//following-sibling::div//span/div"));
		    details.put("OpsCat2", AgentGetText("//label[text()='Ops Cat 2 (New)']//following-sibling::div//span/div"));
		    details.put("OpsCat3", AgentGetText("//label[text()='Ops Cat 3 (New)*']//following-sibling::div//span/div"));
		    details.put("EscalatedTicket", AgentGetText("//label[text()='Escalated Ticket?*']//following-sibling::div//span/div"));
		    details.put("ApprovalWorkflow", AgentGetText("//label[text()='Approval workflow']//following-sibling::div//span/div"));
		    return details;
		}

		
	//Verify the Parent Ticket After Eform Submission
	public static String VerifyParentTicket_AfterIntiation(String Ticket_ID, String HasApproval,
		...
		try {
			...
			if (ParentTicket_Created == true) {
				...
				// Use utility method for bulk data fetching
	            Map<String, String> ticketDetails = fetchTicketDetails();

	            // Combine details into a single line using StringBuilder
	            StringBuilder ticketInfo = new StringBuilder();
	            ticketDetails.forEach((key, value) -> ticketInfo.append(value).append("-"));
	            ticketInfo.append(Get_GetApprovalStatus).append("-").append(ApprovalTicket_ID).append("-").append(Get_TicketStatus);
	            
	            System.out.println(ticketInfo.toString());
	            
	            // Perform assertions using a single method
	            assertEqual(ticketDetails.get("Assignee"), Ticket_Assignee, "Assignee");
	            assertEqual(ticketDetails.get("Priority"), Ticket_Priority, "Priority");
	            assertEqual(ticketDetails.get("HRTicketType"), Ticket_HRTicketType, "HRTicketType");
	            assertEqual(ticketDetails.get("OpsCat1"), Ticket_OpsCat1, "OpsCat1");
	            assertEqual(ticketDetails.get("OpsCat2"), Ticket_OpsCat2, "OpsCat2");
	            assertEqual(ticketDetails.get("OpsCat3"), Ticket_OpsCat3, "OpsCat3");
	            assertEqual(ticketDetails.get("EscalatedTicket"), Ticket_EscalatedTicket, "EscalatedTicket");
	            assertEqual(ticketDetails.get("ApprovalWorkflow"), Number_of_Approval, "ApprovalWorkflow");
	...
	// Utility method to simplify assertions
	private static void assertEqual(String actual, String expected, String fieldName) throws IOException {
		
	    if (actual.equalsIgnoreCase(expected)) {
	        LogPASS(fieldName + " is correctly populated --->" + actual);
	    } else {
	        LogFAIL(fieldName + " is not correctly populated --->" + actual);
	    }
	}
