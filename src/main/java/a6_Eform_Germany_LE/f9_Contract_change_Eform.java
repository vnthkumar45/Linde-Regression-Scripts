...
public class f9_Contract_change_Eform extends AgentGenericMethods2 {
	
	public static void Contract_change_Eform_Verification_and_Submission(String URL, String ScreenshotPath,
		...
		ReadExcel(EformPath);
		try {
			String Login_sheet_name = "DE_Contract_change_Eform";
			...
			for (int i = 1; i <= totalRows; i++) {// declaring while loop to iterate in sheet untill we have rows
				...
				String TypeOfContractRequest = GetdataFromSheet(Login_sheet_name, "Reason for contract change", i);
				...
				if (EformName.isEmpty()) {
					...
					TypeOfRequestfor = TypeOfContractRequest ;
					
					//MyHR - Submit Second Employer Eform
					Class1_MyHR_genericMethods.Populate_fields_and_Submit_Eform(Scenario,"Contract_change", RequestRaisedFor, MethodName);
