...
public class e1_Second_Employer_Eform extends AgentGenericMethods2 {
	
	public static void Second_Employer_Eform_Verification_and_Submission(String URL, String ScreenshotPath,
		...
		ReadExcel(EformPath);
		try {
			String Login_sheet_name = "DE_Second Employer";
			...
			for (int i = 1; i <= totalRows; i++) {// declaring while loop to iterate in sheet untill we have rows
				...
				if (EformName.isEmpty()) {
					...
					Class1_MyHR_genericMethods.Populate_fields_and_Submit_Eform(Scenario,"SecondEmployer", RequestRaisedFor, MethodName);
