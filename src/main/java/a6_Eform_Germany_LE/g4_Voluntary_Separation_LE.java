...
public class g4_Voluntary_Separation_LE extends AgentGenericMethods2 {
	...
	public static void VoluntarySeparation_Eform_Verification_and_Submission(String URL, String ScreenshotPath,
		...
		ReadExcel(EformPath);
		try {
			String Login_sheet_name = "DE_LE_Voluntary Separation";
			...
			for (int i = 1; i <= totalRows; i++) {// declaring while loop to iterate in sheet untill we have rows
				...
				if (EformName.isEmpty()) {
					...
					Class1_MyHR_genericMethods.Populate_fields_and_Submit_Eform(Scenario,"Voluntary_Separation_LE", RequestRaisedFor, MethodName);
