...
public class g1_Transfer_change_of_EE_status_Eform extends AgentGenericMethods2 {
	
	public static void Transfer_change_EE_status_Eform_Verification_and_Submission(String URL, String ScreenshotPath,
		...
		ReadExcel(EformPath);
		try {
			String Login_sheet_name = "DE_Transfer_change of EE Status";
			...
			for (int i = 1; i <= totalRows; i++) {// declaring while loop to iterate in sheet untill we have rows
				...
				if (EformName.isEmpty()) {
					...
					Class1_MyHR_genericMethods.Populate_fields_and_Submit_Eform(Scenario,"Transfer_change of EE status", RequestRaisedFor, MethodName);
