...
public class e8_Onetime_payments_Eform extends AgentGenericMethods2 {
	
	public static void OneTime_Payment_Eform_Verification_and_Submission(String URL, String ScreenshotPath,
		...
		ReadExcel(EformPath);
		try {
			String Login_sheet_name = "DE_OneTime Payement";
			...
			for (int i = 1; i <= totalRows; i++) {// declaring while loop to iterate in sheet untill we have rows
				...
				String TypeOfPaymentRequest = GetdataFromSheet(Login_sheet_name, "Type of payment request", i);
				...
				if (EformName.isEmpty()) {
					...
					TypeOfRequestfor = TypeOfPaymentRequest;

					//MyHR - Submit Second Employer Eform
					Class1_MyHR_genericMethods.Populate_fields_and_Submit_Eform(Scenario,"OneTime_Payment", RequestRaisedFor, MethodName);
					...
					if (Country.equalsIgnoreCase("Germany LG/LCO")
							&& (TypeOfPaymentRequest.equalsIgnoreCase("Special Payments/Awards"))) {
